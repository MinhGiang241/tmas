...
import { getExamExport, getExamTestId } from "@/services/api_services/question_api";
      ...
      <>
        ...
        <div className="w-full max-lg:px-3 mb-5">
          ...
          <div
            ...
            dangerouslySetInnerHTML={{ __html: data?.description || examData?.exam?.description || "" }
            }
          />
          ...
          <div className="flex justify-between items-center mt-6 mb-6">
            <div className="text-sm text-m_neutral_900 flex">
              <Menu className="mr-1" />
              {data?.examVersion?.parts.length ?? examData?.parts.length} {t("part")}
            </div>
            <div className="text-sm text-m_neutral_900 flex">
              <Play className="mr-1" />
              {data?.examVersion?.exam?.examNextQuestion ?? examData?.exam?.examNextQuestion === "FreeByUser"
                ? t("free_change_part")
                : t("part_in_row")}
            </div>
            <div className="text-sm text-m_neutral_900 flex">
              <MessageQuestion className="mr-1 scale-75" />
              {data?.examVersion?.exam?.numberOfQuestions ?? examData?.exam?.numberOfQuestions} {t("quest")}
            </div>
            <div className="text-sm text-m_neutral_900 flex">
              <Cup className="mr-1 scale-75" />
              {data?.examVersion?.exam?.totalPoints ?? examData?.exam?.totalPoints} {t("point")}
            </div>
            <div className="text-sm text-m_neutral_900 flex">
              <Time className="mr-1" />
              {/* {data?.Base?.TimeLimitMinutes ? `${data?.Base?.TimeLimitMinutes} ${t("minute")}` : t("unlimited")} */}
              {data?.examVersion?.exam?.timeLimitMinutes ?? examData?.exam?.timeLimitMinutes
                ? `${data?.examVersion?.exam?.timeLimitMinutes ?? examData?.exam?.timeLimitMinutes} ${t("minute")}`
                : t("unlimited")}
            </div>
            <div className="text-sm text-m_neutral_900 flex">
              <Document className="mr-1" />
              {data?.examVersion?.exam?.examViewQuestionType ?? examData?.exam?.examViewQuestionType === "MultiplePages"
                ? t("all_quest_page")
                : t("quest_per_page")}
            </div>
            <div className="text-sm text-m_neutral_900 flex">
              <Group className="mr-1" />
              {data?.examVersion?.exam?.changePositionQuestion ?? examData?.exam?.changePositionQuestion === false
                ? t("keep_quest_order")
          ...
          <div>
            ...
            {(data?.examVersion?.parts ?? examData?.parts)?.map((x: any, key: any) => (
              // console.log(x,"x");
              <Collapse
                defaultActiveKey={["1"]}
              ...
              >
                ...
                >
                  {((data && data.examQuestions) || (examData && examData.jsonExamQuestions)?.map((x: any) => {
                    var e = JSON.parse(x)
                    console.log(e, "eeeee");
